...
CREATE OR REPLACE PACKAGE spms_manager_management_pkg AS
    ...
    PROCEDURE filled_parking_slots (
        p_datetime VARCHAR2
    );
...
-- Insert new parking lot
    ...
    BEGIN
        ...
        IF
            v_address_id IS NULL
            AND v_parking_lot_id IS NULL
        THEN
...
--RAISE;
    ...
    PROCEDURE filled_parking_slots (
        ...
        v_found    BOOLEAN := FALSE;  -- Flag to check if any slots are found
    BEGIN
    -- Convert input string to TIMESTAMP
        BEGIN
            v_datetime := TO_TIMESTAMP ( p_datetime, 'YYYY-MM-DD HH:MI AM' );
        EXCEPTION
            WHEN OTHERS THEN
                dbms_output.put_line('Date format is not correct. Accepted format: YYYY-MM-DD HH:MI AM/PM');
                RETURN;
        END;

    -- Display filled slots
        dbms_output.put_line('Filled Parking Slots at '
                             || to_char(v_datetime, 'YYYY-MM-DD HH:MI AM')
                             || ':');
        ...
        FOR r IN (
            ...
            WHERE
                v_datetime BETWEEN sb.scheduled_start_time AND sb.scheduled_end_time
            ORDER BY
                ps.parking_slot_id
        ) LOOP
            v_found := TRUE;  -- Set the flag to true indicating that at least one slot is filled
            dbms_output.put_line('Slot ID: '
                                 || r.parking_slot_id
                                 || ', Name: '
                                 || r.slot_name
                                 || ', Floor: '
    ...
    -- Check if no slots were found
        IF NOT v_found THEN
            dbms_output.put_line('No slots booked during this time.');
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            dbms_output.put_line('Unexpected error: ' || sqlerrm);
    END filled_parking_slots;

END spms_manager_management_pkg;
/


--RUN USING PL_MANAGER

--Adding parking lot Validation

--null check
-- parking lot null
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('', '123 Main St','Boston', 'Massachusetts', 'United States', '12345-2344', 40.7128, -74.0060, 10.50);
-- stree addr null
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('City Center', '','Boston', 'Massachusetts', 'United States', '12345-2344', 40.7128, -74.0060, 10.50);
-- state null
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('City Center', '12 sallsa lkk','', 'Massachusetts', 'United States', '12345-2344',1, -49.0, 10.0);
-- city null
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('City Center', '12 sallsa lkk','lksalk', '', 'United States', '12345-2344',1, -49.0, 10.0);
-- zip code null
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('City Center', '12 sallsa lkk','lksalk', 'dkdk', 'United States', NULL,1, -49.0, 10.0);
-- country null
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('City Center', '12 sallsa lkk','lksalk', 'dkdk', '', '12345-2344',1, -49.0, 10.0);
-- latitude null
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('City Center', '12 sallsa lkk','Boston', 'Massachusetts', 'United States', '12345-2344',NULL , -74.0060, 10.50);
-- longitude null
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('City Center', '12 sallsa lkk','Boston', 'Massachusetts', 'United States', '12345-2344',1, NULL, 10.50);
-- pricing per hour null
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('City Center', '12 sallsa lkk','Boston', 'Massachusetts', 'United States', '12345-2344',1, -49.0, NULL);



-- special characters in state
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('City Center', '12 sallsa lkk','Boston', 'Massachus]]etts', 'United States', '12345-2344', 12, -74.0060, 10.50);
-- special characters in city
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('City Center', '12 sallsa lkk','Bos?ton', 'Massachusetts', 'United States', '12345-2344', 12, -74.0060, 10.50);
-- special characters in street address
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('City Center', '12 sallsa l;kk','Boston', 'Massachusetts', 'United States', '12345-2344', 12, -74.0060, 10.50);
-- in country
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('City Center', '12 sallsa lkk','lksalk', 'dkdk', 'klklfd,mm,cz]', '12345-2344',1, -49.0, 10.0);
-- parking lot
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('City Cente[]r', '123 main street','Boston', 'Massachusetts', 'Uniter States', '12345-2344',1, -49.0, 10.0);
--Invalid Zipcode
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('Parking Lot 12', '123 Main St','Boston', 'Massachusetts', 'United States', '12345-234', 40.7128, -74.0060, 10.50);


--Valid
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('Parking Lot 12', '123 Main St','Boston', 'Massachusetts', 'United States', '12345-2344', 89.9, 94.090, 10.50);

--Address Already Exists
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('Parking Lot 12', '123 Main St','Boston', 'Massachusetts', 'United States', '12345-2344', 74.9, 95.090, 12.50);
...
EXEC PLADMIN.spms_manager_management_pkg.spms_add_floor('Parking Lot 12]','123 Simon St','Boston', '20120-6007', 'F1', 10.5);

-- invalid street address
EXEC PLADMIN.spms_manager_management_pkg.spms_add_floor('Parking Lot 12','123 Simo[n St','Boston', '20120-6007', 'F1', 10.5);
-- height exceeds max range
EXEC PLADMIN.spms_manager_management_pkg.spms_add_floor('Parking Lot 12','123 Simon St','Boston', '20120-6007', 'F1', 20);
-- floor level length exceeded
EXEC PLADMIN.spms_manager_management_pkg.spms_add_floor('Parking Lot 12','123 Simon St','Boston', '20120-6007', 'F1saallk;la;las', 10.5);


--null

-- state
EXEC PLADMIN.spms_manager_management_pkg.spms_add_floor('Parking Lot 12','123 Simon St','', '20120-6007', 'F1', 10.5);
-- parking lot name null
EXEC PLADMIN.spms_manager_management_pkg.spms_add_floor('','123 Simon St','Boston', '20120-6007', 'F1', 10.5);
-- streer address null
EXEC PLADMIN.spms_manager_management_pkg.spms_add_floor('Parking Lot 12','','Boston', '20120-6007', 'F1', 10.5);
-- city null
EXEC PLADMIN.spms_manager_management_pkg.spms_add_floor('Parking Lot 12','123 Simon St','', '20120-6007', 'F1', 10.5);
-- slot name null
EXEC PLADMIN.spms_manager_management_pkg.spms_add_floor('Parking Lot 12','123 Simon St','Boston', '20120-6007', '', 10.5);
-- floor height null
EXEC PLADMIN.spms_manager_management_pkg.spms_add_floor('Parking Lot 12','123 Simon St','Boston', '20120-6007', 'F1', '');
...
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_slot('Parking Lot 12', '123 Simon St','Boston', '20120-6007', 'F1', 'SA');



------- ALl Valid case -- 
-- Adding lot/Address
EXEC PLADMIN.spms_manager_management_pkg.spms_add_parking_lot('Parking Lot 12', '123 Simon St','Boston', 'Massachusetts', 'United States', '20120-6007',89.9, 94.090, 10.50);
